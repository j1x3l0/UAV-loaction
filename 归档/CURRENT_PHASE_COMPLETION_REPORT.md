...
不建议立即再跑一轮完整 3×500。优先执行成本更低、针对性更强的 clean 恢复：

1. 从三个 robust-best checkpoint 分别进行短程 clean fine-tuning。
2. 冻结或降低视觉编码器学习率，避免破坏已经获得的尺度鲁棒性。
3. 先运行单 seed 50–100 updates 小试。
4. 使用相同五尺度 × 100 episodes 门控：
   - clean ≥80%；
   - 其他四档均 ≥70%；
   - timeout ≤10%。
5. 若通过，再对三个 seed 执行短程 fine-tuning 和正式 3×5×200 评估。
6. 若 clean 提升伴随极端尺度下降，则改用带 clean 下限约束的
   checkpoint score，而不是继续增加训练量。

完成 clean 缺口修复后，进入 Phase V3c 跨场景泛化；随后开展消融实验和论文
图表整理。
