...
| PX4配置与最终环境对齐 | 进行中 |
| Phase V3 鲁棒训练与 checkpoint 选择 | 0%（正式实验待重做） |
| Phase V3b 消融 | 0%（等待新V3基线） |
| Phase V3c 跨场景泛化 | 0%（等待新V3基线） |
| Phase V4 论文整理 | 约 10% |

当前研究主线尚未形成正式闭环。只有PX4配置、环境对齐验收和新V3复现实验
全部通过后，才能重新评估退化轴、鲁棒checkpoint和多尺度策略是否成立。
