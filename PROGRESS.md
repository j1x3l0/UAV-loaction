...
- ❌ **视觉必要性验证（2026-08-03）未通过**：即使在纯避障 episode（avoid=1.0，直线被障碍挡住）去掉深度也只掉 6-8pp（baseline 22-24% → const_depth 16%）。**深度连避障都不必要**，当前任务撑不起「视觉导航」主张。报告见 `reports/px4_v3_visual_necessity_20260803/`。出路：重设计任务（更密障碍/被遮挡目标/受限向量）或如实弱化视觉主张。
