...
> 更新：随时 | 基于：MASTER_PLAN.md | 当前日期：2026-07-30
...
| **Phase V3d** | 场景几何闭环 | 🔵 同源Gaussian碰撞几何小试运行中 | — |
| **高保真验证** | PX4/Gazebo→Isaac/Pegasus | 🔵 PX4适配层测试通过，SITL待部署 | — |
| **Phase V4** | 论文 | ⬜ 未开始 | — |

> 状态标记：⬜ 未开始 | 🔵 进行中 | ✅ 已完成 | ❌ 已放弃 | ⏸️ 暂停 | ⚠️ 受阻

### 历史结果有效性标记（2026-07-29）
...
| V3d 之后 | 🔵 最终主结果候选 | 场景视觉、点云碰撞、边界、运动相机和连通自由空间对齐后的新实验 | 在视觉必要性门控通过前扩展正式3×500或跨场景结论 |

旧 V1/V2 数据不删除，统一作为 `legacy-unaligned` 历史对照。最终正文中的
真实性、退化临界点和鲁棒训练结论必须由 V3d 对齐环境重新验证；旧结果只能
用于代码演进、内部有效性和“错位环境会产生误导性高SR”的方法学分析。
