...
- ✅ 30 位姿图片已分批补齐（每批 3–5 张、逐批核对数量+大小），服务器图片目录现 33 张、约 27 MB。
- ✅ 30 位姿、64×64 中心裁剪 GPU 注册统计完成：正确 OpenGL→OpenCV 转换覆盖率 1.000、MAE 0.1846、亮度相关 0.1201；错误候选 0.984、0.1941、-0.0256，`provisional_pass=true`。30/30 覆盖率 1.0；逐位姿亮度判别 22/30 正确轴占优。
- ✅ 飞行体积净空门禁通过：映射后 PX4 飞行区域（场景飞行带）在 0.45 m 操作净空下连通自由空间 153.3 m³（判据 10 m³）。净空≥0.25/0.45/0.75/1.00/1.50 m 时连通自由空间分别为 221/153/75/30/0.1 m³；1.5 m 近乎为零符合窄走廊场景设计。
...
- ❌ **视觉必要性验证（2026-08-03）未通过**：即使在纯避障 episode（avoid=1.0，直线被障碍挡住）去掉深度也只掉 6-8pp（baseline 22-24% → const_depth 16%）。**深度连避障都不必要**，当前任务撑不起「视觉导航」主张。报告见 `reports/px4_v3_visual_necessity_20260803/`。
- ✅ **决策（2026-08-03，弱化方向）**：论文先走「如实弱化视觉主张」路线——深度视觉如实报告为中等贡献，主张聚焦「对齐窄视场深度相机下的鲁棒训练分析」+「PX4↔3DGS 对齐方法论」。**视觉必要性重设计（更密障碍/被遮挡目标）作为后续可选工作**，不阻塞当前论文主线。

### 后续执行顺序（2026-08-02）

1. ✅ 剩余图片分批 3–5 张上传，每批核对数量和大小。
...
## 视觉必要性与两周期冲刺（2026-08-03~04）

- ✅ **D3-lite 视觉必要性门控通过**（合成稠密走廊，2026-08-03）：单 seed baseline 54% / const_depth 0%；**5-seed 4/5 显著**（配对 McNemar p<0.001），诚实标注 seed3 向量泄露（无深度也 100%）与 seed1 部分必要（42%）。D1（碰撞半径）/D2（去向量）失败后，**障碍密度是诱导深度必要性的正确杠杆**。报告见 `reports/px4_d3lite_visual_necessity_20260803/`。
- ✅ **对齐 5 轴退化曲线补齐**（2026-08-03）：对齐环境下重做 V2 退化评估。**新发现：视角不确定性是关键脆弱轴**（clean 基线 45° 视角掉到 18%），curriculum 鲁棒训练显著改善（54% 不掉）；分辨率/稀疏化/噪声中度，光照不变（深度-only 负对照）。报告见 `reports/px4_v3_aligned_degradation_20260803/`。
- ✅ **clean SR 攻关**（2026-08-04）：3000ep clean 达到 **66.0%**（1500ep 56.7% +9.3pp），未达 70% 目标；**reward 调优空间已记录**（下一步杠杆）。
- 🔄 **curriculum 3000ep 长训**（2026-08-04 进行中）：5 seeds × 3000ep，与 3000ep clean 公平对比（同等算力下 clean vs robust），预计 ~12-15h。
- ✅ **两周 ICRA 冲刺计划**：`docs/two-week-icra-plan.md`（D1-D14 门控，ICRA 2027 主攻）。
- ✅ 论文主线完整：**必要性（D3-lite）+ 退化相变（0.25x）+ 修复（curriculum）+ 对齐方法论 + 视角轴发现**。
