...
def calculate_max_mean_gradient(parameters):
    ...
    for param in parameters:
        if param.grad is not None:
            gradients.append(param.grad.view(-1))  # Flatten gradient

    if gradients:
        gradients = torch.cat(gradients)  # Combine all gradients into one tensor
        mean_gradient = torch.mean(torch.abs(gradients))  # Calculate mean of absolute gradients
        max_gradient = torch.max(torch.abs(gradients))    # Calculate max of absolute gradients
        return max_gradient, mean_gradient
    else:
        return None, None  # If no gradients are present
