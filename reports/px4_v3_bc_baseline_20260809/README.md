...
| 方法 | SR | 说明 |
|------|:---:|------|
| **BC（行为克隆，3 seeds）** | **32.0%** | 监督学习复现 oracle |
| **RL（PPO clean 5 seeds）** | **53.2%** | 端到端强化学习 |

**BC 32.0% < RL 53.2%（−21.2pp）**。

## 结论

1. **RL 优于 BC（−21.2pp，3 seeds 一致）**：强化学习的闭环适应（探索 + 奖励信号）
   优于行为克隆的静态监督复现。BC 从 oracle 复现"应该怎么做"，但无法像 RL 一样
   在交互中学习对观测噪声/初始状态扰动的鲁棒应对。3 seeds（34/30/32%）方向一致，
   无倒挂。
2. **BC 基线有意义**：32% 证明任务可被监督学习部分解决，但达不到 RL 水平——
   支持"RL 是视觉导航正确方法"的论文主张。
3. **oracle 98% vs RL 53.2% vs BC 32%**：oracle 有全局路径信息（上帝视角），远超
   学习策略；但 BC（从 oracle 学）只达 32%，说明 oracle 动作无法被端到端复现
   （视觉歧义、分布漂移）。
